...
public class Monstre extends Pion implements NonTraversable {
    ...
    /**
     * Renvoie la liste des directions visibles par le zombie, c'est-à-dire celles dans lesquelles il peut se déplacer.
     * @return 
     */
    ...
    /** 
     * En prenant une direction, donne vers cette dernière la distance de la nourriture la plus proche.
     * @param d : la direction dans laquelle on doit chercher à manger.
     * @return la distance du repas. S'il n'y en a pas dans cette direction, retourne -1.
     */
    ...
    /**
     * Le zombie regarde où est le jeton le plus proche, et met à jour sa direction en fonction de ce dernier.
     */
    public void regarder() {
        ...
        for (int i = 0; i<3; i++) {
            if ((this.trouverAManger(l.get(i)) < distance) && this.trouverAManger(l.get(i)) > 0) { //Puis on regarde si les autres directions proposent de la nourriture plus près.
                ...
                this.direction = l.get(i) ; //Une fois qu'on a trouvé la direction la plus intéressante, on MAJ la direction du zombie.
